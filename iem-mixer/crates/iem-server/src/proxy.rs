...
pub async fn batch_control(
    ...
    match payload.operation {
        ...
        BatchOperation::MuteAll => {
            ...
            {
                let elev_id = member_id.clone();
                let is_elev = state.elevated_store.read().await.is_elevated(&elev_id);
                    ...
                    let mix_urls: Vec<String> = discovered
                        ...
                        .filter_map(|m| {
                            let si = if member_id == "engineer" {
                                ...
                                m.mix_send_indices.get(&elev_id as &str).copied()
                            };
...
async fn apply_command_to_cache(
    ...
    let elevated_check_id = member_id.clone();
    let is_elevated = state
        .elevated_store
        .read()
        .await
        .is_elevated(&elevated_check_id);
    let mix_members: Vec<(usize, Option<usize>)> = if member_id == "engineer" {
        ...
        discovered
            ...
            .filter(|m| m.id() != member_id && m.id() != "engineer")
            .map(|m| {
                let si = m.mix_send_indices.get(&elevated_check_id as &str).copied();
                (m.track_index, si)
            })
            .collect()
    } else {
